...
- Tailwind CSS 4 is configured in src/styles/index.css using `@import "tailwindcss"` and `@plugin` directives for daisyUI 5 and typography.
...
- Navigation uses React Router's Link for internal routes and anchor tags for external URLs.
...
- Tests: pnpm test
- Deploy to GitHub Pages: pnpm deploy (uses gh-pages on dist)
